...
# 
# 【概要】
#  このページでは、「エスペラント文の汉字(日本語)置換」で使う
#  最終的な JSON ファイル(合并3個JSONファイル)を生成する機能を提供します。
# 
# 【主な流れ】
#  1) CSVファイルを取り込み(エスペラント語根→汉字/日本語訳の対応)
...
#---------------------------------------------------------------------
# 例: an, on は後の処理で文字列(汉字)と紐づけるためのサンプルデータ
...
# main.py での文字列(汉字)置換で衝突や誤置換が起こらないように
...
#=====================================================================
st.set_page_config(
    page_title="Công cụ tạo tệp JSON để thay thế văn bản (chữ Hán) trong Esperanto",
    layout="wide"
)
st.title("Tạo tệp JSON dùng để thay thế (chữ Hán) trong văn bản Esperanto.")
st.write("---")

#=====================================================================
# 2) 概要説明 (使い方)
#=====================================================================
with st.expander("Mở hướng dẫn sử dụng", expanded=True):
    st.markdown("""
    #### Giới thiệu
    Trang này cho phép tạo và tải xuống tệp JSON dùng để thay thế trong văn bản Esperanto
    (trang chính). Dung lượng có thể đến khoảng 50MB.

    **Quy trình sử dụng**:
    1. Tải lên hoặc sử dụng mặc định một **tệp CSV** (bảng tương ứng giữa gốc từ Esperanto và bản dịch tiếng Việt, v.v.).
    2. Tải lên hoặc sử dụng mặc định một **tệp JSON** (quy tắc phân tách gốc từ hay cài đặt chuỗi thay thế).
    3. Nhấn nút “Tạo tệp JSON để thay thế” để tải xuống
       **tệp JSON thay thế** đã được tạo.

    Bên dưới có sẵn một số tệp ví dụ. Quý vị có thể tham khảo cách định dạng
    để tùy chỉnh cài đặt theo ý muốn.
    """)
...
# ダウンロードするボタンを設けています。
#=====================================================================
with st.expander("Danh sách tệp ví dụ (có thể tải về)"):
    st.write("#### Danh sách tệp ví dụ")

    # サンプルCSV１
    st.markdown("""
    **Ví dụ CSV 1 (Danh sách gốc từ Esperanto và bản dịch tiếng Việt kèm chú thích ruby)**  
    Đây là tệp CSV chứa từng dòng tương ứng gốc từ Esperanto và nghĩa tiếng Việt.  
    Chỉ cần tạo CSV theo định dạng này rồi tải lên, bạn sẽ tạo được tệp JSON để thay thế.
    """)
    file_path0 = './Appの运行に使用する各类文件/Danh sách gốc từ Esperanto với bản dịch tiếng Việt và chú thích ruby.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Tải ví dụ CSV 1 (Gốc Esperanto - Bản dịch tiếng Việt + chú thích ruby)",
            data=file,
            file_name="GocTuEsperanto_BanDichTiengViet_Ruby.csv",
            mime="text/csv"
        )

    # サンプルCSV２
    st.markdown("""
    **Ví dụ CSV 2 (Gốc từ Esperanto và chữ Hán, đề xuất của Mingeo trên Zhihu)**  
    Đây là tệp CSV ghép gốc Esperanto với chữ Hán.
    """)
    file_path0 = './Appの运行に使用する各类文件/Mingeo先生版 世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Tải ví dụ CSV 2 (Gốc Esperanto - Chữ Hán, của Mingeo)",
            data=file,
            file_name="GocEsperanto_ChuHan_Mingeo.csv",
            mime="text/csv"
        )

    # サンプルCSV３
    st.markdown("""
    **Ví dụ CSV 3 (Gốc từ Esperanto và chữ Hán)**  
    Đây là tệp CSV ghép gốc Esperanto và chữ Hán.
    """)
    file_path0 = './Appの运行に使用する各类文件/世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Tải ví dụ CSV 3 (Gốc từ Esperanto - Chữ Hán)",
            data=file,
            file_name="GocEsperanto_ChuHan.csv",
            mime="text/csv"
        )

    # サンプルJSON１
    st.markdown("""
    **Ví dụ JSON 1 (Cài đặt người dùng về quy tắc phân tách gốc từ Esperanto)**  
    **Mục đích**: quy định cách phân tách gốc từ Esperanto, thứ tự thay thế,
    hoặc thời điểm áp dụng các hậu tố (như hậu tố động từ).  
    Tham khảo phần chú thích trong tệp ví dụ.
    (Ví dụ: `["am", "dflt", ["verbo_s1"]]` )
    """)
    ...
    with open(json_file_path, "rb") as file_json:
        btn_json = st.download_button(
            label="Tải ví dụ JSON 1 (Cài đặt quy tắc phân tách gốc từ Esperanto)",
            data=file_json,
            file_name="CaiDatPhanTachGocTuEsperanto.json",
            mime="application/json"
        )

    # サンプルJSON２
    st.markdown("""
    **Ví dụ JSON 2 (Cài đặt người dùng về chuỗi sau khi thay thế)**  
    **Mục đích**: Áp dụng với một số từ nhất định, ngoài quy tắc phân tách gốc ở trên,
    nếu cần chỉ định chữ Hán hoặc cách viết đặc thù.  
    (Thông thường chỉ cần chỉnh CSV và/hoặc JSON phân tách gốc là đủ.)
    """)
    json_file_path2 = './Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json'
    with open(json_file_path2, "rb") as file_json:
        btn_json = st.download_button(
            label="Tải ví dụ JSON 2 (Cài đặt chuỗi sau khi thay thế)",
            data=file_json,
            file_name="CaiDatChuoiSauKhiThayThe.json",
            mime="application/json"
        )

    st.markdown("""
    **Ví dụ Excel 1 (Danh sách gốc từ Esperanto và chú thích dịch ở 14 ngôn ngữ)**  
    (Tiếng Nhật, tiếng Trung, tiếng Hàn, tiếng Anh, tiếng Nga, tiếng Tây Ban Nha,
    tiếng Ý, tiếng Pháp, tiếng Đức, tiếng Ả Rập, tiếng Hindi, tiếng Ba Lan,
    ...
    with open('./Appの运行に使用する各类文件/Kreado de rubenaj komentoj en 14 lingvoj (日本語, 中文, 한국어, English, Русский, español, italiano, français, Deutsch, العربية, हिन्दी, polski, Tiếng Việt, Bahasa Indonesia) respondaj al listo de 4736 Esperant.xlsx', "rb") as file:
        st.download_button(
            label="Tải ví dụ Excel 1 (Gốc Esperanto với 14 ngôn ngữ)",
            data=file,
            file_name="DanhSachGocTuEsperanto_VaChuThich_14NgonNgu.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )

    # サンプルExcel
    st.markdown("""
    **Ví dụ Excel 2 (Gốc từ Esperanto và bản dịch tiếng Nhật (kèm cấp độ học), v.v.)**  
    **Mục đích**: hữu ích khi muốn tùy chỉnh thêm phần chú thích ruby, 
    nêu cấp độ học dựa trên Từ điển cơ bản Esperanto-Nhật, v.v.
    """)
    with open('./Appの运行に使用する各类文件/エスペラント語根-日本語訳ルビ対応リスト(習得レベル付き).xlsx', "rb") as file:
        st.download_button(
            label="Tải ví dụ Excel 2 (Gốc Esperanto - Dịch Nhật + cấp độ học)",
            data=file,
            file_name="GocEsperanto_DichNhat_CapDoHoc.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
...
#=====================================================================
options = {
    "HTML (điều chỉnh kích thước chữ Ruby)": "HTML格式_Ruby文字_大小调整",
    "HTML (điều chỉnh kích thước chữ Ruby + thay thế chữ Hán)": "HTML格式_Ruby文字_大小调整_汉字替换",
    "HTML (cơ bản)": "HTML格式",
    "HTML (thay thế chữ Hán)": "HTML格式_汉字替换",
    "Dạng ngoặc": "括弧(号)格式",
    "Dạng ngoặc (thay thế chữ Hán)": "括弧(号)格式_汉字替换",
    "Thay thế đơn giản": "替换后文字列のみ(仅)保留(简单替换)"
}


display_options = list(options.keys())
selected_display = st.selectbox('Chọn kiểu đầu ra (output):', display_options)
format_type = options[selected_display]

# Ví dụ đơn giản: main_text_list và ruby_content_list
...
st.write("---")
st.markdown("**Văn bản đã được định dạng ⇓**")
...
#=====================================================================
st.header("Bước 1: Chuẩn bị tệp CSV")
st.markdown(
    """
### Chọn **tệp CSV** chứa gốc từ Esperanto và thông tin dịch (chữ Hán) cần thay thế.

---
    """
)

csv_choice = st.radio("Bạn muốn xử lý tệp CSV như thế nào?", ("Tải lên tệp CSV", "Sử dụng mặc định"))
csv_path_default = "./Appの运行に使用する各类文件/Danh sách gốc từ Esperanto với bản dịch tiếng Việt và chú thích ruby.csv"

CSV_data_imported = None

if csv_choice == "Tải lên tệp CSV":
    st.write("Vui lòng tải lên bất kỳ tệp CSV nào (khuyến nghị dùng UTF-8).")
    ...
    if uploaded_file is not None:
        ...
        st.success("Đã tải lên tệp CSV thành công.")
    else:
        st.warning("Chưa tải lên tệp CSV nào.")
        st.stop()

elif csv_choice == "Sử dụng mặc định":
    try:
        ...
        csv_buffer = StringIO(converted_text)
        CSV_data_imported = pd.read_csv(csv_buffer, encoding="utf-8", usecols=[0, 1])
        st.info("Đang sử dụng tệp CSV mặc định.")
    except FileNotFoundError:
        st.error("Không tìm thấy tệp CSV mặc định. Dừng xử lý.")
        st.stop()

st.write("Đã nạp xong tệp CSV. Tiếp tục bước kế tiếp.")
st.write("---")

#=====================================================================
# ステップ2: JSONファイル(語根分解法など)を準備
#=====================================================================
st.header("Bước 2: Chuẩn bị tệp JSON (quy tắc phân tách gốc từ, v.v.)")
st.markdown("""
Bạn có thể tải lên hoặc sử dụng mặc định các tệp JSON
chứa **quy tắc phân tách gốc từ Esperanto** và/hoặc
**chuỗi thay thế tùy chỉnh**.

Bạn có thể tải tệp JSON mẫu về và chỉnh sửa, sau đó tải lên.
""")

json_choice = st.radio("1. Chọn tệp JSON về quy tắc phân tách gốc từ Esperanto?", ("Tải lên tệp JSON", "Sử dụng mặc định"))
json_path_default = "./Appの运行に使用する各类文件/世界语单词词根分解方法の使用者自定义设置.json"
custom_stemming_setting_list = None

if json_choice == "Tải lên tệp JSON":
    uploaded_json = st.file_uploader("Tải lên tệp JSON (quy tắc phân tách gốc)", type=['json'])
    if uploaded_json is not None:
        custom_stemming_setting_list = json.load(uploaded_json)
        st.success("Đã tải lên tệp JSON thành công.")
    else:
        st.warning("Chưa tải lên tệp JSON nào.")
        st.stop()
elif json_choice == "Sử dụng mặc định":
    try:
        with open(json_path_default, "r", encoding="utf-8") as g:
            custom_stemming_setting_list = json.load(g)
        st.info("Đang sử dụng tệp JSON mặc định.")
    except FileNotFoundError:
        st.error("Không tìm thấy tệp JSON mặc định.")
        st.stop()

json_choice2 = st.radio("2. Chọn tệp JSON về chuỗi thay thế riêng?", ("Tải lên tệp JSON", "Sử dụng mặc định"))
json_path_default2 = "./Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json"
user_replacement_item_setting_list = None

if json_choice2 == "Tải lên tệp JSON":
    uploaded_json = st.file_uploader("Tải lên tệp JSON (chuỗi thay thế)", type=['json'])
    if uploaded_json is not None:
        ...
        st.warning("Chưa tải lên tệp JSON nào.")
        st.stop()
elif json_choice2 == "Sử dụng mặc định":
    try:
...
# ステップ3: 高度な設定 (並列処理)
#=====================================================================
st.header("Bước 3: Cài đặt nâng cao (xử lý song song)")
with st.expander("Mở cài đặt về xử lý song song"):
    st.write("""
    Tại đây, bạn có thể cài đặt số tiến trình xử lý (sử dụng nhiều core CPU)
    để tạo tệp JSON thay thế. Nếu số gốc từ hoặc văn bản rất lớn,
    việc chia nhỏ xử lý song song có thể tăng tốc độ.
    """)

    use_parallel = st.checkbox("Sử dụng xử lý song song", value=False)
    num_processes = st.number_input("Số tiến trình chạy đồng thời", min_value=2, max_value=6, value=5, step=1)

st.write("### Tạo tệp JSON cuối cùng để thay thế (nút bấm)")

#=====================================================================
# 置換用JSONファイルを作成するボタン
#=====================================================================
if st.button("Tạo tệp JSON để thay thế"):
    with st.spinner("Đang tạo tệp JSON để thay thế... Vui lòng chờ trong giây lát."):
        #-------------------------------------------------------------
        ...
        #     「エスペラント語根 → (汉字 or ベトナム語訳ルビ等)」を反映させる
        #-------------------------------------------------------------
        ...
        if use_parallel:
            ...
            for i, j in enumerate(E_stem_with_Part_Of_Speech_list):
                ...
                if i % 1000 == 0:
                    ...
                    progress_text.write(f"{current_count}/{total_items} đối tượng đang được xử lý...")

            progress_bar.progress(100)
            progress_text.write("Phần xử lý dài nhất đã hoàn thành 100%. (Cần thêm khoảng 3~4 giây nữa.)")
        ...
        st.success("Đã hoàn tất việc tạo danh sách thay thế!")

        st.download_button(
            label="Tải xuống danh sách thay thế cuối cùng (kết hợp 3 tệp JSON)",
            data=download_data,
            file_name="DanhSachThayThe_CuoiCung_HopNhat3JSON.json",
            mime='application/json'
        )
